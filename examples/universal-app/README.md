# Numbers Vectorization App

This example turns `vectorizeRasterLine` from `src/index.js` into a React UI and
uses the same build output for GitHub Pages, Electron desktop packages, and
Capacitor mobile projects.

The app includes 50 MNIST training samples generated by
`experiments/prepare-mnist-samples.mjs` and a canvas surface for pointer or Apple
Pencil input. The canvas path is rasterized first with `getImageData`; the
vectorizer receives pixels, not pointer coordinates.
...
To regenerate the compact MNIST sample module:
